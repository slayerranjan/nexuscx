...
try {
  ...
  try {
    await db.execute(`ALTER TABLE agents ADD COLUMN is_super_admin INTEGER NOT NULL DEFAULT 0;`);
  } catch {
    // already exists — safe to ignore
  }

  try {
    await db.execute(`ALTER TABLE conversations ADD COLUMN agent_typing_until TEXT;`);
  } catch {
    // already exists — safe to ignore
  }
